...
var LocalizedStrings_1 = createCommonjsModule(function (module, exports) {
...
var LocalizedStrings = function () {
  ...
  function LocalizedStrings(props, options) {
    ...
    this._opts = _extends({}, {
      customLanguageInterface: utils$1.getInterfaceLanguage,
      pseudo: false,
  ...
  _createClass(LocalizedStrings, [{
    ...
    value: function setContent(props) {
      ...
      this._props = props;
      utils$1.validateTranslationKeys(Object.keys(props[this._defaultLanguage]));
      ...
      Object.keys(obj).forEach(function (property) {
        if (_typeof(obj[property]) === "object") {
          ...
          for (var i = 0; i < strArr.length; i += 1) {
            if (strArr[i].match(placeholderReplaceRegex)) ; else if (strArr[i].match(placeholderReferenceRegex)) ; else {
              var len = strArr[i].length;
              if (_this2._opts.pseudoMultipleLanguages) {
                len = parseInt(len * 1.4, 10); // add length with 40%
              }
              strArr[i] = utils$1.randomPseudo(len);
            }
          }
    ...
    value: function setLanguage(language) {
      ...
      // should be used
      var bestLanguage = utils$1.getBestMatchingLanguage(language, this._props);
...
var __pika_web_default_export_for_treeshaking__ = /*@__PURE__*/getDefaultExportFromCjs(LocalizedStrings_1);
